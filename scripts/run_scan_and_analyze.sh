#!/usr/bin/env bash
# Runs one discovery scan, one analysis pass, and one resolution check.
# Intended to be invoked by systemd (see deploy/polymarket-companion.service)
# on a recurring schedule so market_snapshots/features/analyses/resolutions
# history actually accumulates over time without manual intervention.
#
